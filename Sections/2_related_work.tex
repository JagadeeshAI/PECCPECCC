...
\textcolor{red}{LMEE-Bench~\cite{wang2026memoryexplorer} is the setting most directly related to ours, and the one our formulation is derived from. It couples multi-goal navigation with memory-based question answering: a single instruction names several objects, \emph{all} of which are terminal goals to be reached in sequence, and the agent is afterwards queried about what it observed along the way. MemoryExplorer addresses this task by reinforcement fine-tuning a 7B multimodal LLM with a multi-task reward over action prediction, frontier selection, and question answering, so that the agent learns to query its memory actively while exploring. We depart from it in two respects. First, we reformulate the task rather than the model: only one named object is a goal, and the remaining references are landmarks whose role is to be \emph{verified} rather than visited, which turns them into a source of feedback instead of a list of destinations. Second, we require no training. Where MemoryExplorer adapts across episodes through gradient updates, our agent adapts within a single episode from its own prediction errors, at no training cost.}
